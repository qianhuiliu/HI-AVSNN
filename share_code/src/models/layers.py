...
class Layer_inAttention(nn.Module):
    def __init__(self,in_plane,out_plane,kernel_size,stride,thres):
        super(Layer_inAttention, self).__init__()
        self.fwd = SeqToANNContainer(
            nn.Conv1d(in_plane,out_plane,kernel_size,stride,bias=False),
            nn.BatchNorm1d(out_plane)
        )
        self.act = LIFSpike(thresh = thres)
